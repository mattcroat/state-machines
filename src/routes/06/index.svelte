<script>
  ...
  onMount(() => {
    ...
    const countDrags = assign({
      drags: (context, state) => (context.drags += 1),
    })

    const notMaxDrags = (context, state) => {
    ...
    service.onTransition((state) => {
      if (state.changed) {
        ...
        boxEl.dataset.drags = state.context.drags
...
<h1>Exercise 06 - Guarded Transitions</h1>
